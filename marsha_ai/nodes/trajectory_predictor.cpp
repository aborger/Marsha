...
class KalmanFilter {
    private:
        ...
        const Matrix3f R; // R = Measurement Uncertainty (Measurement noise covariance matrix)
    ...
    public:
        ...
        // An initial estimate at how certain the measurement is. (Initial measurement should be good as the initial state is throwing arm end effector position)
        ...
        void predict(const Vector6f x_c, const Vector3f u, float dt) {
        ...
        // u = control vector (measured acceleration)
        // dt = change in time
        void update(const Vector3f z, const Vector3f u, float dt) {
            // Update kalman gain
